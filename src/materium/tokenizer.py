...
class BERTOSPredictor:
    ...
    def predict_formula(self, formula: str) -> List[str]:
        """
        Predicts oxidation states from a formula string.
        Returns: List of species strings like ['Fe|+3', 'O|-2', ...]
        """
        ...
        input_seq = ""
        elements_in_order = []
        for ele, count in comp_dict.items():
        ...
        # Convert predictions to species tokens: "Element|+Charge"
        predicted_species = []
        for i, ele in enumerate(elements_in_order):
            os_val = true_pred[i].item() - 5
            predicted_species.append(f"{ele}|{os_val:+d}")
...
class TOSSPredictor:
    def __init__(self, toss_path):
        """
        toss_path: Absolute path to the TOSS directory containing .pkl files.
        """
    ...
    def predict_structure(self, structure: Structure) -> Structure:
        """
        Takes a pymatgen Structure, runs TOSS via a temporary CIF, 
