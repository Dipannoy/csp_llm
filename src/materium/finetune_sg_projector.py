...
if __name__ == "__main__":
    ...
    if "space_group" not in model.cond_proj_layers:
        raise RuntimeError(
            f"Condition 'space_group' not found in checkpoint. "
            f"Available: {list(model.cond_proj_layers.keys())}"
        )
    sg_layer = model.cond_proj_layers["space_group"]
    if not isinstance(sg_layer, SpaceGroupEmbedding):
        raise RuntimeError(
            f"Expected SpaceGroupEmbedding for 'space_group', got {type(sg_layer)}. "
            "Run migrate_sg_fingerprint.py first."
        )
    ...
    train_crystals = CrystalDataset(
        train_dataset,
        tokenizer,
        cache_path=f"v3_oxy_crystal_token_cache_latticelast_{args.dataset}_{args.oxidation_mode}_train_so{tokenizer.sorting_order.value}_seq{tokenizer.sequence_order}.joblib",
        recalculate_cache=False,
    )
    val_crystals = CrystalDataset(
        val_dataset,
        tokenizer,
        cache_path=f"v3_oxy_crystal_token_cache_latticelast_{args.dataset}_{args.oxidation_mode}_test_so{tokenizer.sorting_order.value}_seq{tokenizer.sequence_order}.joblib",
        recalculate_cache=False,
    )
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        train_loss = run_epoch(model, optimizer, train_loader, DEVICE, is_train=True,
                                                              tokenizer=tokenizer,
                               keep_conditions={"space_group"} if args.freeze_projector_only else None)
